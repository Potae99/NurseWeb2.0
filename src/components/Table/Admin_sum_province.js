...
function Admin_sum_province() {
  ...
  useEffect(() => {
    const fetchData = () => {
      ...
      axios.get(process.env.REACT_APP_API_URL + "/summary/student/province/count")
        .then(res => {
    ...
    return filterAdmin.slice(start, end).map((_, index) => (
      <tbody key={start + index}>
        <tr className="hover:bg-gray-200 bg-white border-b">
          ...
          <td className="py-4 px-6">{_.province}</td>
...
export default Admin_sum_province