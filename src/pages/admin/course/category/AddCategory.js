...
function AddCategory() {
  ...
  const handleDeleteCategoryClick = async (categoryID) => {
    const categoryToDelete = category.find(
      (item) => item.categoryID === categoryID
    );

    if (
      categoryToDelete &&
      categoryToDelete.categoryName === "General"
    ) {
      Swal.fire({
        title: "ข้อมูลนี้ไม่สามารถลบได้",
        icon: "warning",
      ...
      const result = await Swal.fire({
        title: "ต้องการลบหมวดวิชาหรือไม่?",
        showDenyButton: true,
        showCancelButton: true,
        confirmButtonText: "ใช่",
        denyButtonText: `ไม่ใช่`,
      ...
      if (result.isConfirmed) {
        ...
        if (results.isConfirmed) {
          try {
            await axios.delete(
              process.env.REACT_APP_API_URL + "/course/category",
              {
                data: { categoryID: categoryID },
              }
            );

            const updatedCategoryList = categoryList.filter((item) => item.categoryID !== categoryID);
      ...
      {!completed ? (
        ...
        <div className=' text-black bg-white min-h-screen'>
          ...
          <div className=' mt-7 grid grid-cols-2 '>
            <div className=' ml-3'>
              <button onClick={() => backToCourseAll()} className="relative inline-flex items-center justify-center p-4 px-6 py-3 overflow-hidden font-medium text-black transition duration-300 ease-out border-2 border-orange-400 rounded-full shadow-md group">
                <span className="absolute inset-0 flex items-center justify-center w-full h-full text-white duration-300 -translate-x-full bg-orange-400 group-hover:translate-x-0 ease">
